 ...
 * Saída: tmp/testes-pecas-scaffold/<run-id>/ (PDF forense, mesma formatação do app)
 */
...
async function mainAsync() {
  ...
  console.log(`Lote ${estado.proximoIndice}…${fim - 1} (PDF) → ${outRoot}`);
  ...
  for (let i = estado.proximoIndice; i < fim; i++) {
    ...
    try {
      ...
      const pdf = await gerarPecaPdfBuffer(peca);
      writeFileSync(join(dirArea, nomePdf), pdf);
      // Metadado leve ao lado (não abre no leitor de PDF)
      writeFileSync(
        ...
        [
          ...
          `Gerado: scaffold PDF (0 tokens redação)`,
          vazou9099
      ...
      ok++;
      linhasIndice.push(
        `- [${i}] ${job.areaId}/${nomePdf}${vazou9099 ? " ⚠️ 9.099" : ""}`
      );
  ...
  appendFileSync(
    logPath,
    `\n[${new Date().toISOString()}] lote ${estado.proximoIndice}-${fim - 1} pdf ok=${ok} falhas=${falhas}\n${linhasIndice.join("\n")}\n`,
    "utf8"
  );
  ...
  const prev = existsSync(readmePath)
    ...
    : `# Testes scaffold FACTO (${estado.runId})\n\nPDF forense (mesma formatação do app). 0 tokens de redação.\n\n`;
  writeFileSync(
    readmePath,
    prev +
      `\n## Lote ${estado.proximoIndice}–${fim - 1} (PDF)\n\n` +
      linhasIndice.join("\n") +
      "\n",
    "utf8"
  );
