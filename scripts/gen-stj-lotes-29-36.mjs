/**
 * Gera lotes STJ 29–36 (281–360) + seed.
 * Uso: node scripts/gen-stj-lotes-29-36.mjs
 */
import { writeFileSync } from "fs";

const cancelMeta = {
  321: "CANCELADA pela Segunda Seção em 24/02/2016 (Projeto de Súmula n. 627, DJe 29/02/2016).",
  343: "CANCELADA pela Primeira Seção em 28/04/2021 (QO no MS 7.078/DF, Projeto de Súmula n. 700, DJe 03/05/2021).",
  348: "CANCELADA pela Corte Especial em 17/03/2010 (CC 107.635/PR, DJe 23/03/2010).",
  357: "REVOGADA pela Primeira Seção em 27/05/2009 (REsp 1.074.799/MG, DJe 22/06/2009).",
};
...
for (let lote = 29; lote <= 36; lote++) {
  const start = 281 + (lote - 29) * 10;
  const end = start + 9;
  ...
  const headerNote = canceladas.length
    ? `Cancelada(s)/revogada(s): ${canceladas.join(" e ")} (não entram no RAG ativo).`
    : "ativas — VerbetesSTJ.";
  let extra = "";
  if (start === 301) extra = "\n * Obs.: Súmula 309 usa a redação alterada em 22/03/2006.";
  if (start === 321) extra = "\n * Obs.: Súmula 323 usa a redação alterada em 25/11/2009.";
 ...
 * Canceladas/revogadas fora do RAG: ${canceladasAll.join(", ")}.
...
writeFileSync("scripts/seed-sumulas-stj-lote-29-36.mjs", seed, "utf8");
