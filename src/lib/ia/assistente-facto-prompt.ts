...
export function montarSystemPromptRedacaoTier1(
  ...
  return [
    ...
    "   - Em DAS PROVAS E ANEXOS, liste os documentos como 1)/2)/3)/… (NÃO use a)/b)/c)), em texto normal (sem negrito), sem linha em branco entre os itens.",
    "   - Em DO DIREITO, cada subtítulo a)/b)/c) fica sozinho, em negrito, com 1 linha em branco antes do próximo subtítulo.",
    ...
    "   - Liste documentos/mídias como 1)/2)/3)/… (texto normal, sem negrito).",
