...
export function urlLogoEmail(): string {
  return `${getSiteUrl()}/brand/facto-logo-email.png`;
}

/**
 * Bloco HTML da logo (para e-mails escuros ou claros — fundo transparente).
 * height ~56px no cliente; wordmark + ícone empilhados.
 */
export function htmlLogoEmail(opcoes?: {
  ...
  return `<img src="${src}" alt="${alt}" width="${Math.round(h * 1.27)}" height="${h}" style="display:inline-block;height:${h}px;width:auto;max-width:200px;border:0;outline:none;text-decoration:none;" />`;
}
