...
Módulo aberto hoje: **todas as áreas com dashboard** (`available: true`), salvo **Contratual** (fora da grade; litígio no Civil; Contratos em breve). Inclui **Constitucional**. Qualquer plano ativo (Essencial/`jec`, Completo, Pro, trial, escritório) libera **todas** as áreas do catálogo; diferenças = cota, Sonnet (Essencial 10% · Completo 20% · Pro 26% · trial 0%) e export no trial. OAB opcional no perfil. Eleitoral aberto no catálogo, lastro TRE/TSE ainda fraco.
...
- **Formatação vs conteúdo:** a IA tem liberdade total para redigir fatos, direito e pedidos. O código só aperfeiçoa **formatação visual** (margens NBR, tipografia, espaços, citações `[[JURIS]]`, rodapé numérico, layout do cabeçalho). Não reescrever dados da peça.
- **Cabeçalho forense:** espécies em `idsPeticaoInicial` (e equivalentes) = molde de **qualificação completa** (polo ativo → nome forense → `em face de` + passivo). **Endereçamento inaugural:** sempre `DA ___ª VARA …` (ex.: `DA ___ª VARA CÍVEL`), nunca “DE UMA DAS VARAS” nem “DA VARA” sem underline — a distribuição ainda não ocorreu. Incidentais = só nomes + **epígrafe com Processo nº** quando o número estiver no dossiê. JT = reclamação; JECRIM = queixa; Penal = HC/revisão; etc. — nome forense do rito, sem engessar o mérito.
- **Citações:** só copy-paste literal **integral** (ementa/enunciado ipsis litteris) da base FACTO em `[[JURIS]]`; interpretação/cabimento = corpo normal. Lastro estadual = TJ da UF do foro (+ STF/STJ/TST/TSE/TRF/TRT quando couber); **nunca** TJ de outro Estado.
- **Rodapé:** número da página no canto inferior direito (não “Folha X de Y”).
...
- **Redator:** Flash padrão. Claude Sonnet Essencial **10%** / Completo **20%** / Pro **26%** só com **função detalhada aceita** no chat (O5b) + gatilho + `ANTHROPIC_API_KEY`. Trial = Flash. Sem botões Expressa/Equilíbrio/Detalhada no header — pergunta no chat quando o roteador recomenda e há teto. **Auto-crítica (O8):** 1 pass Flash pós-Redator só no DO DIREITO (não enxuga; fail-open); flag `AUTOCRITICA_DIREITO_ATIVA`.
- **Endereçamento fiel aos autos:** não inventar «Vara Cível»/«Família» se o PDF só traz «1ª VARA»; especialidade só se explícita. Default de `areaJudiciaria` não força mais JEC quando `areaId` está setado. **Fidelidade gênero:** se autos só trazem filha (ou só filho), prompt + pós-process corrigem generalização masculina/feminina.
...
- **Peça no painel:** só após **Gerar preview** (1 crédito) / durante o stream da IA. Área **Peça** (ex-Minuta). Plano na aba Plano. Correções: edição inline (contenteditable) ou chat (`ajuste_peca` / **Aplicar ajuste**).
...
- **Operação no chat (08/09):** modos **Assistente** / **Peça**; **Direto** / **Aprofundado**; **Usar meu modelo**; **Gerar preview** / **Montar a peça**; pós-peça = **Aplicar ajuste** + chips. Crédito só no Gerar preview confirmado. **Sem** paleta `/`.
...
- Epígrafe em peça incidental: Processo nº + polos do rito (Autor/Réu; Apelante/Apelado; Recorrente/Recorrido; Agravante/Agravado; Embargante/Embargado; Exequente/Executado; Reclamante/Reclamado; Reconvinte/Reconvindo; Impetrante/Paciente ou Impetrado; Querelante/Querelado; Acusado/MP; Representante/Representado). Petições curtas (`memorial`, `manifestacao`, `juntada`, `ciencia`, `informacoes`…) → só Processo nº. Em rótulo recursal, a 1ª linha é quem protocola (polo do advogado). PDF e DOCX desenham as linhas no bloco de 6. Endereçamento usa foro/comarca; incidental sem vara não leva `___`. Alvo do recurso (r. sentença/decisão) fica no parágrafo introdutório (`ante a r. sentença…`), não em segundo `em face de` após o título.
...
- **Último ato / peça cabível:** heurística local **não escolhe** a espécie no chat (só extrai último ato / alerta no Auditor). Peça = IA ou chips do advogado. `pecaCabivelAposUltimoAto` permanece para Auditor (alerta) e testes de calibração — não para forçar redator.
- **Skins visíveis:** Maestro, Analista, Pesquisa & súmulas, Estrategista, Redator, Auditor. Sem 7ª skin. Sem chamada Gemini extra. Maestro monta o plano com a espécie **já confirmada** (IA/chips). Pesquisa busca na base FACTO **casos semelhantes aos fatos** e **julgados favoráveis ao polo da peça**, filtrados pelo **TJ da UF do foro** (+ superiores/regionais cabíveis). Estrategista injeta `<VINCULOS_FACTO>` sem remap local de remédio.
- **Chat esclarecimento:** chips clicáveis (peça + polo) após organizar o caso — sem interrogatório longo; documentos/chat contextualizam a redação.
