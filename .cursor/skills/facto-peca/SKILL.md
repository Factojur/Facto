...
- **Peça cabível:** a **IA interpreta** o caso (área, espécie, último ato). Heurística local é **pista fraca** (extração de partes/metadados) — não força remédio. Cumprimento/execução já instaurado + interlocutória → em regra **agravo de instrumento** (CPC 1.015, p.ú.); MS só com pedido/sinais explícitos ou decisão da IA. Nunca reabrir o incidente.
- **Chat = produto principal (paridade MinutaIA):** liberdade total de interpretação no chat. Área/módulo FACTO é catálogo + rito **depois** da IA — não trava confirmação de área/polo nem bloqueia Redigir. Dashboards manuais por área existem, mas **não condicionam** o chat. Diferenças vs MinutaIA: **lastro** (base + anexos) e **visual** FACTO.
